...
function getdirname($path = null) {
	if (!empty($path)) {
		if (strpos($path, '\\') !== false) {//strpos() 函数查找字符串在另一字符串中第一次出现的位置。
			return substr($path, 0, strrpos($path, '\\')) . '/';//substr() 函数返回字符串的一部分。
...
function GetVerify($str, $app = null) {
	...
	//return substr(md5(mt_rand().$str . $app . $GLOBALS['pwServer']['HTTP_USER_AGENT']), 16, 16);
