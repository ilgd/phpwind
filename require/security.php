<?php
!defined('P_W') && exit('Forbidden');//exit() 函数输出一条消息，并退出当前脚本。
/**
...
class S {
	 ...
	 */
	function str($param) {
		return trim($param);//从字符串的两端删除空白字符和其他预定义字符
	}
	/**
	 * 是否对象
	 * @param $param
	...
	function isObj($param) {
		return is_object($param) ? true : false;//是否为类类型
	}
	/**
	 * 是否数组
	 * @param $params
	...
	function isNatualValue($param){
		switch (gettype($param)) {
			...
			case 'integer':
			case 'double':
				return strlen($param) > 0;//strlen() 函数返回字符串的长度。如果长度大于0,返回true,否则false
			break;
			case 'array':
	...
	function stripTags($param) {
		return trim(strip_tags($param));//剥去字符串中的 HTML 标签：
	}
	/**
	 * 初始化$_GET/$_POST全局变量
	 * @param $keys
	...
	function gp($keys, $method = null, $cvtype = 1,$istrim = true) {
		...
		foreach ($keys as $key) {
			if ($key == 'GLOBALS') continue;//continue 在循环结构用用来跳过本次循环中剩余的代码并在条件求值为真时开始执行下一次循环。
			$GLOBALS[$key] = NULL;
